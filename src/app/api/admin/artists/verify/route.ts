...
export async function POST(request: Request) {
  try {
    ...
    const parseResult = kycReviewSchema.safeParse(body);
    ...
    const { artist_id, status, rejection_reason } = parseResult.data;

    // 4. Update artist record
    ...
    const { error: logError } = await supabase.from("audit_logs").insert({
      admin_id: user.id,
      action: `VERIFY_ARTIST_${status.toUpperCase()}`,
      details: { artist_id, rejection_reason: status === "rejected" ? rejection_reason : null },
      ip_address: ipAddress,
    });

    if (logError) {
      console.error("Error writing audit log:", logError);
      // Don't fail the request if just the audit log insert fails, but log it
    }

    return NextResponse.json({
      success: true,
      message: `Le dossier KYC de l’artiste a été ${status === "approved" ? "approuvé" : "rejeté"} avec succès.`,
    });
  } catch (error: any) {
    return NextResponse.json(
